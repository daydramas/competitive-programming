...
int main() {
	...
	multiset<string> ss;
	EACH(s, A) ss.insert(s);
	int aa = 0, bb = 0;
	EACH(s, B) aa += (bool)ss.count(s);
	EACH(s, C) bb += (bool)ss.count(s);
	// debug(A);
	// debug(B);
	// debug(C);
	// EACH(s, C) { cout << (bool) ss.count(s) << ' '; }
	// cout << '\n';
	double aaa = 1.00 * aa / B.size();
	double bbb = 1.00 * bb / C.size();
	if(aaa < bbb) print("NUMBER ONE IS THE HUMAN");
	else print("NUMBER TWO IS THE HUMAN");
	// cout << aa << ' ' << bb << '\n';
	// debug(aaa,bbb);
}