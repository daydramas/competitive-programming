/* modified code from http://www.usaco.org/current/data/sol_cowland_gold_feb19.html */
#include <bits/stdc++.h>
...
int segtreeqry(int idx, int l, int r, int lhs, int rhs) {
  ...
  int ret = 0;
  int m = (l+r)/2;
  if(m >= lhs) ret = max(ret,segtreeqry(2*idx, l, m, lhs, rhs));
  if(m+1 <= rhs) ret = max(ret,segtreeqry(2*idx+1, m+1, r, lhs, rhs));
  return ret;
}
...
int pathQuery(int child, int par) {
  ...
  while(child != par) {
    if(topchain[child] == child) {
      // light edge
      ret = max(ret,vals[child]);
      child = lca[child][0];
    }
    else if(depth[topchain[child]] > depth[par]) {
      ret = max(ret, segtreeqry(vertextosegtree[topchain[child]], vertextosegtree[child]));
      child = lca[topchain[child]][0];
    }
    else {
      ret = max(ret,segtreeqry(vertextosegtree[par]+1, vertextosegtree[child]));
      break;
    }
  }
  return ret;
...
int query(int a, int b) {
  int r = getLCA(a, b);
  return max(max(pathQuery(a, r),pathQuery(b, r)), vals[r]);
}
...
int main() {
  ...
  for(int i=0; i<4*MAX_N; i++) segtree[i]=1e9+7;
  for(int i = 1; i < n; i++) {
    int a, b;
    cin >> a >> b;
    a--; b--;
