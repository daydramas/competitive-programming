...
struct segTree {
	...
	void build(int id, int L, int R, vi &a) {
        ...
        l[id]=max(l[left(id)], r[left(id)]);
        r[id]=max(l[right(id)], r[right(id)]);
	}

	void update(int id, int L, int R, int P, int value) {
		if (P > R || P < L) return;
		if (L == R) { l[id]=r[id]=value; return; }
	...
	int get(int id, int L, int R, int N) {
        if (l[id] < N && r[id] < N) return 0;
        if (L==R) return L;
        if (l[id] >= N) return get(left(id), L, (R+L)/2, N);
        if (r[id] >= N) return get(right(id), (R+L)/2+1, R, N);
		return 0;
	}
